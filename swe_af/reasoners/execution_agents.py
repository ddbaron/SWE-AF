...
async def run_retry_advisor(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.retry_advisor", ai_provider),
...
async def run_issue_advisor(
    ...
    async def _invoke_advisor(
        ...
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.issue_advisor", ai_provider),
            tools=["Read", "Write", "Glob", "Grep", "Bash"],
            cwd=cwd,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
    ...
    async def _invoke_replanner(
        ...
        for attempt in range(2):
            result = await router.harness(
                ...
                model=replan_model,
                provider=provider,
                profile=profile_for("swe_af.main.replanner", ai_provider),
                tools=["Read", "Write", "Glob", "Grep", "Bash"],
...
async def run_issue_writer(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.issue_writer", ai_provider),
...
async def run_verifier(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.verifier", ai_provider),
            tools=["Read", "Write", "Glob", "Grep", "Bash"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_git_init(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.git_init", ai_provider),
            tools=["Bash", "Write"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_workspace_setup(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.workspace_setup", ai_provider),
            tools=["Bash", "Write"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_merger(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.merger", ai_provider),
...
async def run_integration_tester(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.integration_tester", ai_provider),
            tools=["Bash", "Read", "Write", "Glob", "Grep"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_workspace_cleanup(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.workspace_cleanup", ai_provider),
            tools=["Bash", "Write"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_coder(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.coder", ai_provider),
...
async def run_qa(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.qa", ai_provider),
            tools=["Read", "Write", "Edit", "Bash", "Glob", "Grep"],
            cwd=worktree_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_code_reviewer(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.code_reviewer", ai_provider),
            tools=["Read", "Write", "Glob", "Grep", "Bash"],
            cwd=worktree_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def generate_fix_issues(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.fix_issues", ai_provider),
            tools=["Read", "Write", "Glob", "Grep", "Bash"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_repo_finalize(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.repo_finalize", ai_provider),
            tools=["Bash", "Read", "Write", "Glob", "Grep"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_github_pr(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.github_pr", ai_provider),
            tools=["Bash", "Write"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
...
async def run_ci_fixer(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.ci_fixer", ai_provider),
...
async def run_pr_resolver(
    ...
    try:
        result = await router.harness(
            ...
            model=model,
            provider=provider,
            profile=profile_for("swe_af.main.pr_resolver", ai_provider),
            tools=["Bash", "Read", "Edit", "Write", "Glob", "Grep"],
            cwd=repo_path,
            max_turns=DEFAULT_AGENT_MAX_TURNS,
            permission_mode=permission_mode or None,
